...
class DocumentList extends React.Component {
  ...
  async componentDidMount() {
    const { viewingSeminar, setStudentContainer } = this.props;
    const student_mails = [];
  ...
  makeEmail(file, mails = []) {
    const { viewingEvent, profile, viewingSeminar } = this.props;
  ...
  renderItem(d, i) {
    const {
      classes, hasCheckbox, handleCheckbox, sendStudents, profile,
    } = this.props;
          ...
          expandControl[i] && (
            <div className={classes.editBar} onClick={() => this.handleClick(i)}>
              {permission.sendToAllStudents && student_mails && sendStudents && (
                ...
                >
                  <img className={classes.iconSpeaker} src="https://image.ibb.co/dD2xK9/speaker.png" alt="img" />
                  发给所有学生
                </Button>
              )}
              ...
              {permission.deleteFile && (
                <Button
                  ...
                  classes={{ label: classes.label }}
                >
                  <TrashCan className={classes.icon} />
                  移除
                </Button>
              )}
            </div>
